...
function    startEnabled(btn)   {
    btn.style.color = "#fff";
    btn.style.border = "2px solid rgb(0,155,0)";
...
function checkForWinCon()   {
    ...
    info = info.split(','); // Separarte the string by commas into an array
    info.sort(); // Sort the square array in order despite the actual gameplay sequence
    for (var i in info)     {
        squareArray.push(info[i].charAt(0)); // New array with ony squares not avatar
    }
...
// Call this function to check board state for any ties and act accodingly
function    check4Tie()     {
...
function    square1Animate()    {
    ...
    if (activePlayer != "Game Stopped") { // If game has not yet started, prevent avatar placement
        ...
        if (verdict == undefined)   { // If verdict is empty, then the square is unoccupied
            ...
            avatarPlaced(square,paintAvatar); // End current turn and pass turn to other player
            squareSound(); // Play the sound when the avatar is placed 
